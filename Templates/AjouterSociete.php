...
if(isset($_REQUEST['Enrgistrer']))
{
    ...
    $ICE	= $_REQUEST['txt_ICE'];
		...
		try
		{
			if(!isset($errorMsg))
			{
				$insert_stmt=$db->prepare('INSERT INTO Societe(Raison_sociale,Adresse,Ville,Pays,Tel,FAX,EMAIL,Num_post,Identification_fiscale,Patente,Banque,Compte_bancaire,Forme_jurdrique,RC,ICE) 
                VALUES(:Raison_sociale,:Adresse,:Ville,:Pays,:Tel,:FAX,:EMAIL,:Num_post,:Identification_fiscale,:Patente,:Banque,:Compte_bancaire,:Forme_jurdrique,:RC,:ICE)'); //sql insert query					
				$insert_stmt->bindParam(':Raison_sociale',$Raison_sociale);
				$insert_stmt->bindParam(':Adresse',$Adresse);   //bind all parameter
                ...
                $insert_stmt->bindParam(':Num_post',$Num_post);
                $insert_stmt->bindParam(':Identification_fiscale',$Identification_fiscale);
                $insert_stmt->bindParam(':Patente',$Patente);
                $insert_stmt->bindParam(':Banque',$Banque);
                $insert_stmt->bindParam(':Compte_bancaire',$Compte_bancaire);
                $insert_stmt->bindParam(':Forme_jurdrique',$Forme_jurdrique);
                $insert_stmt->bindParam(':RC',$RC);
                $insert_stmt->bindParam(':ICE',$ICE);
